...
type DevicePacket = { id: number; source: string; value: string; time: string };
...
export default function Home() {
  ...
  const [micLevel, setMicLevel] = useState(34);
  const [packets, setPackets] = useState<DevicePacket[]>([{id:1,source:"ESP32",value:"BOOT_OK · local mode",time:"00:00.014"},{id:2,source:"I2S_MIC",value:"noise floor calibrated",time:"00:00.226"},{id:3,source:"BUTTON",value:"interrupt attached GPIO 14",time:"00:00.231"}]);
  ...
  useEffect(() => { if (!deviceOnline) return; const timer=window.setInterval(()=>setMicLevel(18+Math.floor(Math.random()*45)),850); return()=>window.clearInterval(timer); },[deviceOnline]);
  ...
  function capture(event: FormEvent) {
    ...
    setPackets(items=>[{id:Date.now(),source:"NODE",value:`CAPTURE_PACKET · ${input.trim().length} bytes`,time:new Date().toLocaleTimeString([], {minute:"2-digit",second:"2-digit"})},...items].slice(0,5));
  }
  function simulateButton(){if(!deviceOnline||processing)return;setInput("What if a physical device could preserve an unfinished thought before it disappears?");setToast("SIMULATED BUTTON INTERRUPT");setPackets(items=>[{id:Date.now(),source:"BUTTON",value:"RISING_EDGE · GPIO 14",time:new Date().toLocaleTimeString([], {minute:"2-digit",second:"2-digit"})},...items].slice(0,5));}

  return (
    <main>
      <div className="space" aria-hidden="true"><i/><i/><i/></div>
      <header className="topbar">
        <button className="brand" onClick={() => setView("lab")}><span className="brand-orbit"><b/></span><span>THE CORTEX NODE</span></button>
        ...
        <div className="online"><span/> ESP32 DIGITAL TWIN ONLINE</div>
      </header>
        ...
        <>
          <section className="hero" id="laboratory">
            <div className="hero-copy">
              <div className="eyebrow"><span>◇</span> EDGE COGNITION HARDWARE SIMULATOR</div>
              <h1><span>THE CORTEX</span> NODE</h1>
              <h2>A physical interrupt for a disappearing thought.</h2>
              <p>An ESP32-based cognitive capture device turns a button press and microphone input into private memory packets. This browser build is an honest digital twin of the proposed hardware.</p>
            ...
            <div className="observatory" aria-label="Interactive visualization of the Cortex thought network">
              ...
              <div className="callout left">ESP32<br/>EDGE MCU</div><div className="callout right top">I2S MIC<br/>INPUT</div><div className="callout right bottom">LOCAL<br/>MEMORY</div>
            </div>
          </section>
          <section className="device-lab"><article><header>HARDWARE DIGITAL TWIN <b>{deviceOnline?"● SIMULATOR ONLINE":"○ POWER OFF"}</b></header><div className="board"><strong>ESP32<br/>WROOM-32</strong><button onClick={simulateButton} disabled={!deviceOnline}>PRESS TO<br/>CAPTURE</button></div><footer><button onClick={()=>setDeviceOnline(v=>!v)}>{deviceOnline?"POWER OFF":"POWER ON"}</button><span>GPIO 14 · INTERRUPT</span></footer></article><article><header>I2S MICROPHONE <b>INMP441 · SIMULATED</b></header><div className="mic"><strong>{deviceOnline?micLevel:0}</strong><span>dBFS INPUT LEVEL</span><i style={{width:`${deviceOnline?micLevel:0}%`}}/></div><div className="pins">WS 25 · SCK 26 · SD 33</div></article><article><header>SERIAL MONITOR <b>115200 BAUD</b></header><div className="serial">{packets.map(p=><p key={p.id}><time>{p.time}</time> <b>[{p.source}]</b> {p.value}</p>)}</div><small>SIMULATED TELEMETRY · NO PHYSICAL MEASUREMENTS CLAIMED</small></article></section>
      ...
      <footer id="open-source"><div className="brand"><span className="brand-orbit"><b/></span><span>THE CORTEX NODE</span></div><p>VoltHacks hardware digital twin · firmware and wiring included.</p><span>SIMULATED PROTOTYPE · 2026</span></footer>
    </main>
  );
}
